"""
菜品配图生成：z-image-turbo 单通道
- 主通道：阿里云百炼 DashScope z-image-turbo（multimodal-generation 服务）
- 失败：返回 None → 前端占位图，不阻塞主流程

按 PRD §3.2.3.1 视觉规范：白底高级白盘俯拍

注意：MiniMax image-01 生图已停用（代码整体注释保留备查），
后续配图统一走 DashScope z-image-turbo。
"""
...
# =====================================================================
# MiniMax image-01 生图已停用（2026-08-30），代码注释保留备查，不再调用。
# 配图统一走 DashScope z-image-turbo（见下方 _try_dashscope_image）。
# =====================================================================
...
async def generate_dish_image(
    ...
    通道：DashScope z-image-turbo（主通道，MiniMax image-01 已停用）。
    """
    prompt = _build_prompt(dish_name, ingredients)

    # Mock 模式：不调真实生图 API，返回假 URL（端到端测试用，见 MOCK_IMAGE_GEN）
    if settings.MOCK_IMAGE_GEN:
        return f"https://mock.eat.local/preview/{dish_name}.png"

    # 主通道：DashScope z-image-turbo
    if not settings.DASHSCOPE_API_KEY:
        logger.warning("DASHSCOPE_API_KEY 未配置，跳过生图（前端用占位图）")
        return None
    url = await _try_dashscope_image(prompt)
    if url:
        return url

    logger.warning(f"DashScope z-image-turbo 生图失败: {dish_name!r}")
    return None